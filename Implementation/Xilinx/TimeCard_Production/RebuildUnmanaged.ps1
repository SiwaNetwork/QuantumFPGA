# Скрипт для пересборки TimeCard_Production с неуправляемой конфигурацией

$ErrorActionPreference = 'Stop'

Write-Host "Пересборка TimeCard_Production с неуправляемой конфигурацией..." -ForegroundColor Green
Write-Host "Конфигурация: Неуправляемый DCXO + Signal Generator" -ForegroundColor Yellow
...
$tclScript = "rebuild_unmanaged.tcl"
$tclContent = @"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "`n🎯 Неуправляемая конфигурация применена:" -ForegroundColor Yellow
        ...
        Write-Host "- Clock Detector для контроля входного сигнала" -ForegroundColor Cyan
